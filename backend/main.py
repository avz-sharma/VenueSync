...
def sanitize_personal_data(logger, method_name, event_dict):
    ...
    sensitive_keys = {"personal_data", "ssn", "phone", "email", "raw_incident_notes", "untrusted_incident_note", "pii"}
    for key in list(event_dict.keys()):
        if key.lower() in sensitive_keys or any(s in key.lower() for s in ["personal", "ssn", "email"]):
