...
def compute_rain_shift_targets(
    ...
    if total_remaining_capacity == 0:
        ...
        for idx, c_id in enumerate(covered_ids):
            target_occs[c_id] = target_occs.get(c_id, 0) + split + (1 if idx < rem else 0)
    else:
        # Distribute proportionally based on remaining capacity
        distributed = 0
        sorted_covered = sorted(covered_remaining.items(), key=lambda x: x[1], reverse=True)
        ...
        for idx, (c_id, rem_cap) in enumerate(sorted_covered):
            if idx == len(sorted_covered) - 1:
                ...
                to_add = int(total_exposed_moving * (rem_cap / total_remaining_capacity))
...
class InterventionStateManager:
    ...
    def __init__(self, approved_at: float, source_occupancies: Dict[str, int], target_occupancies: Dict[str, int], duration: float = 12.0):
